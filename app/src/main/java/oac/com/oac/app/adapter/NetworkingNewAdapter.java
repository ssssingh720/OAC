...
/**
 * Created by Sudhir Singh on 22,July,2017
 * ESS,
 * B-65,Sector 63,Noida.
 */
public class NetworkingNewAdapter extends RecyclerView.Adapter<NetworkingNewAdapter.ViewHolder> {

    private Context context;
    private List<Networking> videoDetailList;
    private List<Networking> searchArraylist;

    public NetworkingNewAdapter(Context context, List<Networking> videoDetailList) {
        this.context = context;
        this.videoDetailList = videoDetailList;
        this.searchArraylist = new ArrayList<Networking>();
        this.searchArraylist.addAll(videoDetailList);
    }

    @Override
    public NetworkingNewAdapter.ViewHolder onCreateViewHolder(ViewGroup parent,
                                                              int viewType) {
        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.networking_dtl_adapter, parent, false);

        return new NetworkingNewAdapter.ViewHolder(itemView);

    }

    @Override
    public void onBindViewHolder(NetworkingNewAdapter.ViewHolder holder, int position) {
        ...
        if (videoDetailList.get(position).isSelected()) {
...
//        holder.txtSpeakerDesig.setText(videoDetailList.get(position).getDesignation());
        ...
        holder.lnrNetworkingDtl.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent videoDescription = new Intent(context, NetworkingProfileDetailActivity.class);
...
//        String smsNumber = "917835846055";
    ...
    public void filter(String charText) {
        ...
        if (charText.length() == 0) {
            videoDetailList.addAll(searchArraylist);
        } else {
            for (Networking wp : searchArraylist) {
                if (wp.getName().toLowerCase(Locale.getDefault()).contains(charText)) {
                    videoDetailList.add(wp);
                }
            }
        }
